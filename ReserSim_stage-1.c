...
int nVars = 43, nObjs = 2, nConsts = 0;
...
int year_type			=	INPUT: "Hydrologic year tyoe (dry=1;normal=2;wet=3)"
...
// The necessary IHA subroutines (parameters chosen based on PCA) is invoked here  - employing RVA		
	
	IHA_median_inf and IHA_median_rel		// IHA group-1 monthly medians of July and May
	Extremities_inf and Extremities_rel		// IHA group-2 7-day and 90-day maximum 
	pulse_count_inf and pulse_count_rel		// IHA group-4 low pulse count - LPD
	rates_inf and rates_rel					// IHA group-5 rise rate
	
	float overall_HA_RVA;

	overall_HA_RVA = sqrt((IHA_sum_July + IHA_sum_May	+(HAmid7_max*HAmid7_max) + (HAmid90_max*HAmid90_max)
						+(HA_middle_LPD*HA_middle_LPD) + (HA_middle_rise*HA_middle_rise))/6);
...
// Objective functions
	...
	f[isim] = IrrMSI;
	isim = isim+1;
	f[isim] = (overall_HA_RVA) or (overall_HA_HCA);
}
